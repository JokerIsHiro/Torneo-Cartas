  ...
  const latestDeckByPlayer = useMemo(() => {
    ...
    for (const deck of decklists) {
      ...
      if (!current || deck.updatedAt >= current.updatedAt)
        latestByPlayer.set(deck.playerId, deck);
    }
      ...
      <section className="results-overview">
        <div className="results-title-card">
          ...
          <div>
            ...
            {winner && (
              <p>
                Ganador: {winner.name} con {winner.points} puntos
              </p>
            )}
          </div>
        </div>
        ...
        <div className="results-summary-grid">
          ...
          <SummaryCell
            label="Decklists"
            value={
              tcg === "chess" ? "-" : `${decklistsReceived}/${players.length}`
            }
          />
          <SummaryCell
            label="Rondas"
        ...
        <div className="results-actions-panel">
          ...
          {tcg !== "chess" && (
            ...
            >
              <i className="ti ti-cards" aria-hidden="true" /> Constructor de
              mazos
            </button>
          )}
        </div>
      ...
      {tcg !== "chess" && (
        <div className="results-layout">
          <div className="results-left-column">
            <Standings tournamentId={tournamentId} showHistory={false} />
          </div>
            ...
            <section className="results-decklist-panel">
              <header>
                <div>
                  ...
                  <span>
                    {latestDeckByPlayer.size}/{players.length} recibidas
                  </span>
                </div>
              ...
              {standings.map((row) => {
                ...
                return (
                  <div key={row.player.id} className="results-decklist-row">
                    ...
                    <button
                      onClick={() => openDeckBuilder(row.player.id)}
                      title={`Abrir mazo de ${row.player.name} en el constructor`}
                    >
