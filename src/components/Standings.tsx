...
export function Standings({
  ...
  const { status, tcg, exists } = useTournamentsStore(
    useShallow((s) => {
  ...
  if (!exists || status === "setup") {
    return (
      ...
      >
        <i
          className="ti ti-trophy-off"
          aria-hidden="true"
          style={{ fontSize: "24px" }}
        />
        <div style={{ marginTop: "8px" }}>
          Inicia el torneo para ver la clasificacion
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      {showHistory && standings.length >= 3 && (
        <div
          style={{
            display: "grid",
      ...
      >
        ...
        >
          ...
          <select
            ...
            onChange={(event) =>
              setTiebreakerSystem(
                tournamentId,
                event.target.value as TournamentTiebreakerSystem,
              )
            }
            style={selectStyle}
        ...
        >
          ...
          <span style={{ textAlign: "center" }}>
            {primaryTiebreakerMetric
              ? getTiebreakerMetricLabel(primaryTiebreakerMetric)
              : tiebreakerLabel}
          </span>
        </div>
        ...
        {standings.map((row) => (
            ...
            style={{
              ...
              background:
                row.position % 2 === 0
                  ...
                  : "transparent",
              border:
                phaseMode === "swiss-top" && row.position === topCut
          ...
          >
            ...
            <span
              style={{ fontSize: "13px", fontWeight: 500, textAlign: "center" }}
            >
              {row.player.points}
            </span>
            <span
            ...
            >
              {formatTiebreakerValue(
                primaryTiebreakerMetric,
                getTiebreakerValue(row.tiebreakers, primaryTiebreakerMetric),
              )}
            </span>
          </div>
        ))}
            ...
            return (
              <section key={summary.number} className="history-round-card">
                ...
                <div>
                  ...
                  {!matches.length && (
                    <p>No hay emparejamientos guardados para esta ronda.</p>
                  )}
                </div>
              </section>
            );
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: "12px", color: "var(--color-text-secondary)" }}>
        {player.points} pts · {player.wins}V{" "}
        {hasDraws ? `${player.draws}E ` : ""}
        {player.losses}D
      </div>
    </div>
  );
