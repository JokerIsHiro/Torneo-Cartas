...
type GlobalTab = 'timers' | string  // 'timers' o tournamentId

export default function App() {
  const { tournaments, createTournament, deleteTournament } = useTournamentsStore()
  const { initTimer } = useTimerStore()
...
function TournamentView({ tournament, innerTab, onInnerTabChange }: TournamentViewProps) {
  ...
  return (
    <div>
          ...
          {([
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                color: innerTab === t.id ? 'var(--color-text-primary)' : 'var(--color-text-secondary)',
      ...
      {/* Contenido según estado y pestaña */}
...
function TopTab({ label, active, status, onClick, onClose }: TopTabProps) {
  const dotColor = status === 'active' ? '#1D9E75' : status === 'finished' ? '#854F0B' : 'transparent'
